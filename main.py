...
def connect_mqtt():
    def on_connect(client, userdata, flags, rc, properties):
        if rc == 0:
            ...
            client.subscribe("stefan/house/kpis/outside_temperature")
        else:
    ...
    def on_message(client, userdata, msg):
        ...
        try:
            ...
            elif msg.topic == "stefan/house/kpis/outside_temperature":
                app_name = "outsidetemp"
                icon = TEMPERATURE_ICON
                text = str(int(float(text))) + "°C"

            if app_name is not None and icon is not None:
                try:
                    requests.post(
                        f"http://{awtrix_ip}/api/custom?name={app_name}",
                        json={"text": text, "duration": 5, "icon": icon},
                    )
                    logger.info(f"Posted message to AWTRIX: {text}")
                except Exception as e:
                    logger.error(f"Error while handing or sending gcmb value: {e}")
        except:
...
def quotes():
    while True:
        try:
            ...
            r = requests.post(
                f"http://{awtrix_ip}/api/custom?name=quote",
                json={"text": quote_text, "duration": duration},
            )
...
def math_questions():
    while True:
        try:
            ...
            requests.post(
                f"http://{awtrix_ip}/api/custom?name=math",
                json={"text": text, "duration": 5, "icon": MATH_ICON},
            )
            sleep(30)
        except:
            # Do not log stacktrace
            # logger.exception("Error while sending math question")
            logger.error("Error while sending math question")
            sleep(30)
...
def english():
    ...
    while True:
        try:
            ...
            # Do not log stacktrace
            # logger.exception("Error while sending English question")
            logger.error("Error while sending English question")
            sleep(30)
